...
export default function Tabs({ tabs, defaultValue, className = "", panelClassName = "" }) {
  ...
  return (
    <div className={`flex min-h-0 flex-col ${className}`}>
      <div className="z-10 flex shrink-0 gap-1 border-b border-ink-800 bg-ink-950/95 px-1 pt-1 backdrop-blur">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            <button
              ...
              className={`inline-flex h-10 items-center gap-2 rounded-t-md px-3 text-sm font-medium transition ${
                isActive
